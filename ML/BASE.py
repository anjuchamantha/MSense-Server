...
def split_user_base(df, training_percentage_min, training_percentage_max):
...
def train_and_save(dataframe, feature_group, y_col, training_percentage_min=75,
                   training_percentage_max=80):
    t1 = datetime.now()

    # split test train split
    train_data, test_data = split_user_base(dataframe.copy(), training_percentage_min, training_percentage_max)
    ...
    # results
    rf_score_ws = RandomForest.rf_train_and_save(x_train_ws, y_train_ws, x_test_ws, y_test_ws)
